...
Public Class frmItem
    ...
    Dim DAItemPrice As New DSItemPriceTableAdapters.tblItemPriceTableAdapter
    ...
    Private Sub frmItem_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        Else 'Edit item
            ...
            For Each row As DataRow In TblTemItem.Rows
                ...
                txtBarCode.Text = IIf(TypeOf (row("Barcode")) Is DBNull, "", row("Barcode"))
                TxtSalePrice.Text = IIf(TypeOf (row("SalePrice")) Is DBNull, "", row("SalePrice"))
                ...
                If TypeOf row("CateID") Is DBNull = False Then
                    CboCategory.SelectedValue = row("CateID")
                End If
                ...
                If TypeOf row("ItemUnitID") Is DBNull = False Then
                    CboMainUnit.SelectedValue = row("ItemUnitID")
                End If
                If TypeOf row("subunit") Is DBNull = False Then
                    CboSubUnit.Text = row("subunit")
                End If
                ...
                End If
                If TypeOf row("SalePrice") Is DBNull = False Then
                    TxtSalePrice.Text = row("SalePrice")
                End If
    ...
    Private Sub myRefresh()
        ...
        If lblSaveOption.Text = "0" Then
            ...
            CboCategory.SelectedValue = cateID
...
bm.Height), GraphicsUnit.Pixel)
        ...
        If lblSaveOption.Text = "0" Then
            If MessageBox.Show("Do you want save new item ?", "Save", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = Windows.Forms.DialogResult.Yes Then
                ...
                Try
                    ...
                    item.ItemCode = txtItemName.Text & CboMainUnit.SelectedValue & cbContainer.SelectedValue
                    ...
                    ' (Package Info block)
                    item.CateID = CboCategory.SelectedValue
                    item.ItemUnitID = CboMainUnit.SelectedValue 'UoM                    
                    ...
                    category = cateRepo.GetCateByID(CboCategory.SelectedValue)
    ...
    Private Sub menuSave_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles menuSave.Click
        ...
        ' Validation
        If ValidateTextField(txtItemName, "Item name", ErrorItem) = False Then Exit Sub
        If ValidateTextField(txtBarCode, "Barcode", ErrorItem) = False Then Exit Sub
        If ValidateCombobox(CboCategory, "Category", ErrorItem) = False Then Exit Sub
        If ValidateCombobox(CboMainUnit, "Main Unit", ErrorItem) = False Then Exit Sub
        If ValidateTextField(TxtMainUnitQty, "", ErrorItem) = False Then Exit Sub
        If ValidateCombobox(CboSubUnit, "Sub unit", ErrorItem) = False Then Exit Sub

        If ValidateTextField(TxtSubUnitQty, "Radito Qty", ErrorItem) = False Then Exit Sub
        If ValidateTextField(TxtSalePrice, "Sale price", ErrorItem) = False Then Exit Sub
            ...
            If lblSaveOption.Text = "0" Then
                If MessageBox.Show("Do you want save new item ?", "Save", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = Windows.Forms.DialogResult.Yes Then
                    ...
                    Try
                        ...
                        item.ItemCode = txtItemName.Text & CboMainUnit.SelectedValue & cbContainer.SelectedValue
                        item.Barcode = txtBarCode.Text
                        item.RatioQTY = TxtSubUnitQty.Text
                        ...
                        item.CountryID = cbCountry.SelectedValue
                        item.ItemDescription = txtDescription.Text
                        item.mainunit = CboMainUnit.Text
                        item.mainunit_qty = TxtMainUnitQty.Text
                        item.subunit = CboSubUnit.Text
                        item.subunit_qty = TxtSubUnitQty.Text
                        '--- Cost of item
                        If Trim(TxtCost.Text) = "" Then
                            item.UnitPrice = 0
                        Else
                        ...
                        '*** Manage Priviledge for department to show item                       
                        With gridDepartPriviledge
                            For Each rRow As DataGridViewRow In .Rows
                                Dim chk As DataGridViewCheckBoxCell = CType(rRow.Cells("cIsShow"), DataGridViewCheckBoxCell)
                                Dim myDepartPriviledge As New tblPermissionItemShoworHideForDepart
                        ...
                        '--- Update ItemCount in Category table
                        Dim cate = From qCate In THIDataContext.getTHIDataContext.tblCategories Where qCate.CateID = CInt(CboCategory.SelectedValue)
                        ...
                        itemTransMsgStatus = "True"
                        THIDataContext.getTHIDataContext.Connection.Close()
                        DAItemPrice.UpdateResetPrice(False, USER_ID, item.ItemID)
                        DAItemPrice.InsertNewPrice(item.ItemID, EmptyString(TxtSalePrice.Text), True, False, True, USER_ID, Date.Now.Date)
                    Catch ex As Exception
                        trans.Rollback()
                        itemTransMsgStatus = ex.Message
                        THIDataContext.getTHIDataContext.Connection.Close()
                    ...
                    If itemTransMsgStatus = "True" Then
                        ...
                        'DAItemPrice.UpdateResetPrice (False ,USER_ID , 
                    Else
                        MsgBox("Error update item.", MsgBoxStyle.Critical, "Error : " & itemTransMsgStatus)
                    End If
                End If
            Else ' ========================== Implement for update item to database =========================================================================
                ...
                If MessageBox.Show("Do you want upate this item ?", "Update", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = Windows.Forms.DialogResult.Yes Then
                    ...
                    Try
                        ...
                        myItem.ItemCode = txtItemName.Text & CboMainUnit.SelectedValue & cbContainer.SelectedValue
                        myItem.Barcode = txtBarCode.Text
                        myItem.SalePrice = TxtSalePrice.Text
                        '--- Save image
                        If lblPicFilePath.Text <> "" Then
                        ...
                        '--- Package Info block
                        myItem.CateID = CboCategory.SelectedValue
                        myItem.ItemUnitID = CboMainUnit.SelectedValue
                        myItem.RatioQTY = TxtSubUnitQty.Text
                        myItem.CountryID = cbCountry.SelectedValue
                        myItem.ItemDescription = txtDescription.Text
                        myItem.mainunit = CboMainUnit.Text
                        myItem.mainunit_qty = TxtMainUnitQty.Text
                        myItem.subunit = CboSubUnit.Text
                        myItem.subunit_qty = TxtSubUnitQty.Text
                        '--- Cost of item
                        If Trim(TxtCost.Text) = "" Then
                            myItem.UnitPrice = 0
                        Else
                        ...
                        THIDataContext.getTHIDataContext.SubmitChanges()
                        '--- Update ItemCount in Category table
                        If myCateID <> CboCategory.SelectedValue Then
                        ...
                        itemTransMsgStatus = "True"
                        THIDataContext.getTHIDataContext.Connection.Close()
                        DAItemPrice.UpdateResetPrice(False, USER_ID, myItem.ItemID)
                        DAItemPrice.InsertNewPrice(myItem.ItemID, EmptyString(TxtSalePrice.Text), True, False, True, USER_ID, Date.Now.Date)
                    Catch ex As Exception
                        trans.Rollback()
                        itemTransMsgStatus = ex.Message
                        THIDataContext.getTHIDataContext.Connection.Close()
    ...
    Private Sub BtnUom_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnSetContainMain.Click
    ...
    Private Sub TxtRatioQTY_KeyPress(ByVal sender As System.Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles TxtSubUnitQty.KeyPress
        SetDisableKeyString(e)
    End Sub

 
    Private Sub BtnSetContainSubUnit_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnSetContainSubUnit.Click
        Dim fUoM As New frmItemUnit
        If fUoM.ShowDialog() = Windows.Forms.DialogResult.OK Then
            With CboMainUnit
                .DataSource = UnitItemDataAdapter.GetData
    ...
    Private Sub TxtMainUnitQty_KeyPress(ByVal sender As System.Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles TxtMainUnitQty.KeyPress
        SetDisableKeyString(e)
    End Sub
End Class