...
Public Class UCRequestOrder_V1
    ...
    Private Sub UpdateRequestOrderStatusCFA(ByVal pReqeustID As Integer, ByVal pPOStatusID As Integer)
        ...
        Try
            ...
            myRequestStatusTracking.UpdateRequestStatusDate = Now.Date 'GetDateServer()
            ...
            myRequestHistoryStatusTracking.UpdateRequestStatusDate = Now.Date 'GetDateServer()
    ...
    Private Sub btnReceivedAll_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnReceivedAll.Click
        ...
        System.Windows.Forms.DialogResult.Yes Then
                ...
                Try
                    ...
                    myReceived.ReceivedDate = Now.Date 'GetDateServer()
                    ...
                    myReceivedStatusTracking.UpdateReceivedStatusDate = Now.Date 'GetDateServer()
                    ...
                    If myRequestToDepartID = MAIN_STOCK_DEPART_ID Then
                        ...
                        mytblUsed.UsedDate = Now.Date 'now.date 
                        ...
                        For Each rRow As Janus.Windows.GridEX.GridEXRow In GridItemRequestDetail.GetRows
                            ...
                            If myRequestFromDepartID = MAIN_STOCK_DEPART_ID Or myRequestToDepartID = MAIN_STOCK_DEPART_ID Then
                                ...
                                Dim q1 = (From BBT In THIDataContext.getTHIDataContext.tblBeginBalanceTraces Where BBT.Date.Value.Date = Now.Date And BBT.DepartID = MAIN_STOCK_DEPART_ID And BBT.ItemID = itemID1 Select BBT.BeginBalanceTraceID).Count
                                If q1 = 0 Then
                                    '========================= Register Begin Balance of item (myRequestToDepartID) =========================                        
                                    Dim mytblBeginBalanceTrace As New tblBeginBalanceTrace
                                    mytblBeginBalanceTrace.Date = Now.Date 'GetDateServer().Date
                    ...
                    For Each rRow As Janus.Windows.GridEX.GridEXRow In GridItemRequestDetail.GetRows
                        ...
                        THIDataContext.getTHIDataContext.ExecuteCommand("UPDATE ITEM_PROVIDER_TRANS SET TransDate={0},Status=1 WHERE RequestID={1} AND ItemID={2}", Now.Date, ReqID, Val(rRow.Cells("ItemID").Value))
                        THIDataContext.getTHIDataContext.SubmitChanges()
                        ...
                        If myRequestToDepartID <> MAIN_STOCK_DEPART_ID Then
                            ...
                            Dim q = (From BBT In THIDataContext.getTHIDataContext.tblBeginBalanceTraces Where BBT.Date.Value.Date = Now.Date And BBT.DepartID = myRequestToDepartID And BBT.ItemID = itemID Select BBT.BeginBalanceTraceID).Count
                            If q = 0 Then
                                '========================= Register Begin Balance of item (myRequestToDepartID) =========================                        
                                Dim mytblBeginBalanceTrace As New tblBeginBalanceTrace
                                mytblBeginBalanceTrace.Date = Now.Date 'GetDateServer()
                        ...
                        If myRequestFromDepartID <> MAIN_STOCK_DEPART_ID Then
                            ...
                            Try
                                ...
                                Dim q = (From BBT In THIDataContext.getTHIDataContext.tblBeginBalanceTraces Where BBT.Date.Value.Date = Now.Date And BBT.DepartID = myRequestFromDepartID And BBT.ItemID = itemID Select BBT.BeginBalanceTraceID).Count
                                If q = 0 Then
                                    '========================= Register Begin Balance of item (myRequestToDepartID) =========================                        
                                    Dim mytblBeginBalanceTrace As New tblBeginBalanceTrace
                                    mytblBeginBalanceTrace.Date = Now.Date 'GetDateServer().Date
