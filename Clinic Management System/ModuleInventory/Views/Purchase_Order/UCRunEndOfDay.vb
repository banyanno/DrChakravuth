...
Public Class UCRunEndOfDay
    ...
    Private Sub UCRunEndOfDay_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        gridItemsEndofDayRun.DataSource = EndofDayRunDataAdapter.GetAllRunEndofDayData(Now.Date, Now.Date)
    ...
    Sub RunEndofDayStatus(ByVal pDate As Date)
        'CInt(DEPART_ID)
        If EndofDayRepo.IsRunEndofDayInParticularDay(MAIN_STOCK_DEPART_ID, pDate) = False Then
    ...
    Sub RubEndOfDay()
        ...
        Else
            ...
            If ChBAll.Checked = True Then ' else of block chbAll = false -------------------------------------------------------------------------------------------------------------------------------
                ...
                Dim DAInvenReceiveMaster As New DSInventoryClinicTableAdapters.ITEM_RECIEVE_MASTERTableAdapter
                Dim DAInvenDetail As New DSInventoryClinicTableAdapters.ITEM_RECIEVE_DETIALTableAdapter
                ...
                'Dim tblAdjustHis As DataTable = DA_AdjustHis.GetAdjustListByDate(sysDate, MAIN_STOCK_DEPART_ID) ' CInt(row("DepartID")))
                ...
                '=================== Re-run EOD ===============================
                DA_EODRun.DeleteEODByDepID(MAIN_STOCK_DEPART_ID, sysDate) '(CInt(row("DepartID")), sysDate)
                DA_EODStatus.DeleteEODStatusByDepID(MAIN_STOCK_DEPART_ID, sysDate) '(CInt(row("DepartID")), sysDate)
                DA_EODStatus.InsertNewEODStatusByDepID(sysDate, 1, MAIN_STOCK_DEPART_ID) '(sysDate, 1, CInt(row("DepartID")))
                ...
                ' Receive Item by ID
                For Each rows As DataRow In DAInvenReceiveMaster.SelectReceiveItemByDate(sysDate).Rows
                    For Each rowsdetail As DataRow In DAInvenDetail.SelectReceiveDetail(rows("RECEIVE_ID")).Rows
                        DA_EODRun.InsertReqToEOD(sysDate, MAIN_STOCK_DEPART_ID, CInt(USER_ID), CInt(rowsdetail("ITEM_ID")), 0, CInt(rowsdetail("ITEM_RECEIVE_QTY")), 0, 0)
                    Next
                Next
                Application.DoEvents()
                Threading.Thread.Sleep(100)
                'For Each rowsAdjust As DataRow In DA_AdjustHis.GetAdjustListByDate(MAIN_STOCK_DEPART_ID, sysDate).Rows
                '    MsgBox(CInt(rowsAdjust("Difference")))
                '    DA_EODRun.InsertReqToEOD(sysDate, MAIN_STOCK_DEPART_ID, CInt(USER_ID), CInt(rowsAdjust("ItemID")), 0, 0, 0, CInt(rowsAdjust("Difference")))
                'Next
                ...
                For Each RowUsed As DataRow In DA_Used.GetUseListByDate(sysDate, MAIN_STOCK_DEPART_ID).Rows ' CInt(row("DepartID"))).Rows
                    For Each RowUseDetail As DataRow In DA_UsedDetail.GetUsedDetailListByUsedID(RowUsed("UsedID")).Rows
                        DA_EODRun.InsertReqToEOD(sysDate, MAIN_STOCK_DEPART_ID, CInt(USER_ID), CInt(RowUseDetail("ItemID")), 0, 0, CInt(RowUseDetail("UsedQuantity")), 0)
                ...
                For Each RowAdjust As DataRow In DA_AdjustHis.GetAdjustListByDate(sysDate, MAIN_STOCK_DEPART_ID).Rows 'CInt(row("DepartID"))).Rows
                    DA_EODRun.InsertReqToEOD(sysDate, MAIN_STOCK_DEPART_ID, CInt(USER_ID), CInt(RowAdjust("ItemID")), 0, 0, 0, CInt(RowAdjust("Difference")))
                ...
                For Each RowEOD As DataRow In DA_EODRun.GetCurrentEODToUpdate(MAIN_STOCK_DEPART_ID, sysDate) ')'CInt(row("DepartID")), sysDate)
                    Application.DoEvents()
                    Threading.Thread.Sleep(100)
                    ' MessageBox.Show(row("DepartID") & "  " & RowEOD("ItemID"))
                    Dim BeginingItem As Integer
                    Try
                        BeginingItem = CInt(DA_BeginingTract.GetBeginingItemBalance(MAIN_STOCK_DEPART_ID, sysDate, CInt(RowEOD("ItemID"))))
                    ...
                    For Each RowToUpdate As DataRow In DA_EODRun.GetExistingItemEOD(MAIN_STOCK_DEPART_ID, sysDate, CInt(RowEOD("ItemID"))) 'CInt(row("DepartID")), sysDate, CInt(RowEOD("ItemID")))
                        Application.DoEvents()
                        Threading.Thread.Sleep(100)
                        DA_EODRun.DeleteDateByDepAndItem(MAIN_STOCK_DEPART_ID, sysDate, CInt(RowEOD("ItemID")))
                        'DA_EODRun.DeleteDateByDepAndItem(CInt(row("DepartID")), sysDate, CInt(RowEOD("ItemID")))
                        'MsgBox(CInt(RowToUpdate("Ad")))
                        EndBalance = (BeginingItem + CInt(RowToUpdate("Rcqty")) + CInt(RowToUpdate("Ad"))) - CInt(RowToUpdate("UQty"))
                        DA_EODRun.InsertLastUpdateEOD(sysDate, MAIN_STOCK_DEPART_ID, CInt(USER_ID), CInt(RowEOD("ItemID")), CInt(RowToUpdate("Rqty")), CInt(RowToUpdate("Rcqty")), CInt(RowToUpdate("UQty")), CInt(RowToUpdate("Ad")), BeginingItem, EndBalance)
