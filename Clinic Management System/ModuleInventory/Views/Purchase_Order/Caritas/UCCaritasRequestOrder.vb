        ...
        Try
            ...
            For Each CaritasReqDetail As CARITAS_REQUEST_DETAIL_INVENTORY In lstCaritasReqDetail
                ...
                If CaritasReqDetail.IsReceive = True And CaritasReqDetail.IsReady = False Then
                    ...
                    Dim qBBT = From BBT In THIDataContextStockUpdate.getTHIDataContext.tblBeginBalanceTraces Where BBT.Date.Value.Date = Now.Date And BBT.DepartID = CInt(DEPART_ID) And BBT.ItemID = myItemID
                    ...
                    ' THIDataContextStockUpdate.getTHIDataContext.ExecuteCommand("UPDATE CARITAS_REQUEST_DETAIL_INVENTORY SET ReceivedDateDetail={0} WHERE ItemID={1} and RequestID={2}", now.date , myItemID, CaritasReqDetail.RequestDetailID)
                    '--- Update request Status to close (equals 2) in table CARITAS_REQUEST_INVENTORY for approved
                    THIDataContextStockUpdate.getTHIDataContext.ExecuteCommand("UPDATE CARITAS_REQUEST_INVENTORY SET Status={0},ReceivedDate={1} WHERE RequestID={2}", 2, Now.Date, pRequestID)
                    ...
                    THIDataContextStockUpdate.getTHIDataContext.ExecuteCommand("UPDATE CARITAS_REQUEST_DETAIL_INVENTORY SET IsReady={0} ,ReceiveDateDetail={1} WHERE RequestDetailID={2}", True, Now.Date, pReqDetailID)
                    THIDataContextStockUpdate.getTHIDataContext.SubmitChanges()
